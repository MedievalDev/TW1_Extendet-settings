...
OK = '#43b563'
ERR = '#e06c60'
# Hervorhebung AUF hellem oder unruhigem Inhalt (Karten, Bilder, Pergament):
# Gold geht dort unter. Auswahl kraeftiges Rot, Guide-Marker blinken
# Gruen/Gelb. Nur dafuer - auf dunklem Grund bleibt Gold der Akzent.
HI_SEL = '#ff2a2a'
HI_MARK = '#33ff55'
HI_MARK_2 = '#ffe14d'
CANVAS_BG = '#0f0d0a'
...
def apply_dark_theme(root):
    ...
    # Schieber hellgrau auf dunkler Rinne, gold bei Hover und Ziehen
    for cls in ('Vertical.TScrollbar', 'Horizontal.TScrollbar'):
        ...
        style.map(cls, background=[('pressed', GOLD), ('active', GOLD_HI)],
                  arrowcolor=[('active', GOLD)])
    # Schieberegler und Spinbox (clam zeichnet beide sonst hell)
    ...
    style.map('Horizontal.TScale', background=[('active', GOLD_HI)])
    style.configure('TSpinbox', fieldbackground=FIELD, foreground=INK,
                    background=PANEL, arrowcolor=MUT, bordercolor=LINE,
                    insertcolor=INK, padding=(6, 3), arrowsize=12)
    style.map('TSpinbox', arrowcolor=[('active', GOLD)],
